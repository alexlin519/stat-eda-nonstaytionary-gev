...
# Read Yearly Data
yearly_val <- rast("filtered_yearly_max_val_History_1945_2012.tif")[[1]]
yearly_doy <- rast("filtered_yearly_max_doy_History_1945_2012.tif")[[1]]

# Read Monthly Data
monthly_val <- rast("filtered_monthly_max_val_History_1945_2012.tif")[[1:12]]
...
grid_sf$Relative_Opacity <- 0.4 + 0.7 * (grid_sf$Max_Baseflow / max(grid_sf$Max_Baseflow, na.rm = TRUE))
...
leaflet() %>%
  ...
  # Layer 2: Baseflow Volume Heatmap
  addPolygons(
    data = grid_sf,
    fillColor = ~pal_mag(Max_Baseflow),
    fillOpacity = 0.8,
    color = "#333", weight = 0.5, opacity = 0.8,
    group = "Baseflow Volume Heatmap",
    popup = ~Popup_HTML
  ) %>%
  ...
  addLayersControl(
    baseGroups = c("Timing of Annual Peak", "Baseflow Volume Heatmap"),
  ...
  addLegend(
    pal = pal_mag, 
    values = grid_sf$Max_Baseflow, 
    title = "Max Baseflow (Volume)", 
